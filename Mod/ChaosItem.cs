    ...
    internal class ChaosItem : GlobalItem
    {
        ...
        public override void PostUpdate(Item item)
        {
            ...
            if (chaosManager.IsEffectActive((int)ChaosManager.ChaosEffects.DISCRETIZED_MOVEMENT))
            {
                ...
                // get vanilla vel
                vanillaVel = item.velocity - effectVel;
                // remove effect vel from previous subpixel boost
                item.velocity = vanillaVel;
                Vector2 playerPos = item.position;
                Vector2 roundedPos = new Vector2(MathF.Round(playerPos.X / 16f) * 16, MathF.Round(playerPos.Y / 16f) * 16);
                // fix to ground
                roundedPos.Y += 6;
                subPixel += item.position - roundedPos;
                item.position = roundedPos;
                ...
                effectVel = newVel;
                item.velocity = vanillaVel + effectVel;
                prevPos = roundedPos;
            }
